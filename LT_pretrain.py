...
class LineByLineTextDataset(Dataset):
    def __init__(self, tokenizer: PreTrainedTokenizer, args, file_path: str, block_size=512):
        ...
        self.examples = tokenizer.batch_encode_plus(lines, add_special_tokens=True, model_model_model_max_lengthgthgthgth=block_size)["input_ids"]
...
def main():
    ...
    # pathlist = os.listdir(args.output_dir)
    # pathlist.sort()
    # for ii in pathlist:
    #     model_path = os.path.join(args.output_dir, ii, 'model.pt')
    #     if not os.path.isfile(model_path):
    #         continue
    #     model = torch.load(model_path, map_location=args.device)
    #     zero_rate = see_weight_rate(model)
    #     results = evaluate(args, model, tokenizer)
    #     print('zero result = ', zero_rate)
    #     print(model_path)
